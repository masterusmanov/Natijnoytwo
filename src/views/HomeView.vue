...
// Kemtik qo'shish
const kemtikQoshish = () => {
  if (joriyXona.value && yangiKemtikNom.value && yangiKemtikUzunlik.value && yangiKemtikKenglik.value && yangiKemtikX.value !== '' && yangiKemtikY.value !== '') {
    const kemtik: Kemtik = {
      id: Date.now().toString(),
...
const peregorodkaQoshish = () => {
  if (joriyXona.value && yangiPeregorodkaNom.value && yangiPeregorodkaBirinchi.value && yangiPeregorodkaIkkinchi.value) {
...
const kemtikOchirish = (kemtikId: string) => {
  if (joriyXona.value) {
    joriyXona.value.kemtiklar = joriyXona.value.kemtiklar.filter(k => k.id !== kemtikId)
    chizmaChizish()
  }
}
...
const peregorodkaOchirish = (peregorodkaId: string) => {
  if (joriyXona.value) {
    joriyXona.value.peregorodkalar = joriyXona.value.peregorodkalar.filter(p => p.id !== peregorodkaId)
    chizmaChizish()
  }
}
...
const kemtiklarMaydon = computed(() => {
  if (!joriyXona.value) return 0
  return joriyXona.value.kemtiklar.reduce((sum, k) => sum + (k.uzunlik * k.kenglik), 0)
})

const kemtiklarMaterialMaydon = computed(() => {
  if (!joriyXona.value) return 0
  const foiz = joriyXona.value.mavsumTuri === 'issiq' ? 0.10 : 0.07
...
const kemtikMasofalar = computed(() => {
  ...
  return joriyXona.value.kemtiklar.map(kemtik => {
    ...
    const ong = (joriyXona.value!.uzunlik - kemtik.x - kemtik.uzunlik) * (1 - foiz)
    // Kemtik pastidan devorgacha material masofasi
    const past = (joriyXona.value!.kenglik - kemtik.y - kemtik.kenglik) * (1 - foiz)
...
const umumiyXonaMaydon = computed(() => {
  return xonalar.value.reduce((sum, x) => {
    ...
    const kemtiklar = x.kemtiklar.reduce((s, k) => s + (k.uzunlik * k.kenglik), 0)
...
const umumiyMaterialMaydon = computed(() => {
  return xonalar.value.reduce((sum, x) => {
    ...
    const foiz = x.mavsumTuri === 'issiq' ? 0.10 : 0.07
...
const chizmaChizish = () => {
  ...
  if (joriyXona.value.kemtiklar.length === 0) {
    ...
    joriyXona.value.kemtiklar.forEach((kemtik) => {
      ...
      ctx.rect(kemtikMaterialX + kemtikMaterialW, kemtikMaterialY, -kemtikMaterialW, kemtikMaterialH)
    })
  ...
  ctx.fillText(`Material: ${materialUzunlik}×${materialKenglik}m`, materialStartX + 20, materialStartY + 40)
  
  // Material foizini ko'rsatish
  ctx.font = 'bold 24px Arial'
  ctx.fillStyle = '#166534'
  ctx.fillText(`(-${foizKamayish.value}%)`, materialStartX + 20, materialStartY + 70)
  ...
  joriyXona.value.kemtiklar.forEach((kemtik) => {
    ...
    const ongMasofa = ((joriyXona.value.uzunlik - kemtik.x - kemtik.uzunlik) * (1 - foiz)).toFixed(2)
    const pastMasofa = ((joriyXona.value.kenglik - kemtik.y - kemtik.kenglik) * (1 - foiz)).toFixed(2)
    ...
    if (parseFloat(chapMasofa) > 0.1) {
      ...
      ctx.fillText(`${chapMasofa}m`, materialStartX + (kemtikMaterialX - materialStartX) / 2 - 20, kemtikMaterialY - 25)
    }
    ...
    if (parseFloat(ongMasofa) > 0.1) {
      ...
      ctx.fillStyle = '#DC2626'
      ctx.font = 'bold 16px Arial'
      ctx.fillText(`${ongMasofa}m`, kemtikMaterialX + kemtikMaterialW + (materialStartX + materialWidth - kemtikMaterialX - kemtikMaterialW) / 2 - 20, kemtikMaterialY - 25)
    }
    ...
    if (parseFloat(yuqoriMasofa) > 0.1) {
      ...
      ctx.fillStyle = '#DC2626'
      ctx.font = 'bold 16px Arial'
      ctx.translate(kemtikMaterialX - 20, materialStartY + (kemtikMaterialY - materialStartY) / 2)
    ...
    if (parseFloat(pastMasofa) > 0.1) {
      ...
      ctx.fillStyle = '#DC2626'
      ctx.font = 'bold 16px Arial'
      ctx.translate(kemtikMaterialX - 20, kemtikMaterialY + kemtikMaterialH + (materialStartY + materialHeight - kemtikMaterialY - kemtikMaterialH) / 2)
      ctx.rotate(-Math.PI / 2)
    ...
    ctx.fillText(`Kemtik: ${kemtik.uzunlik}×${kemtik.kenglik}m`, kemtikMaterialX + kemtikMaterialW / 2, kemtikMaterialY + kemtikMaterialH / 2)
    ctx.textAlign = 'left'
  })
  ...
  joriyXona.value.kemtiklar.forEach((kemtik) => {
    ...
    ctx.font = 'bold 15px Arial'
    ctx.textAlign = 'center'
    ctx.fillText(`${kemtik.uzunlik}×${kemtik.kenglik}m`, kemtikX + kemtikW / 2, kemtikY - 10)
    ctx.textAlign = 'left'
    
    // Kemtik koordinatalarini ko'rsatish
    ctx.fillStyle = '#7F1D1D'
    ctx.font = '13px Arial'
    ctx.fillText(`X:${kemtik.x}m Y:${kemtik.y}m`, kemtikX + 3, kemtikY + kemtikH + 18)
  })
  ...
  joriyXona.value.peregorodkalar.forEach((per, index) => {
    peregorodkaY = startY + (roomHeight / (joriyXona.value!.peregorodkalar.length + 1)) * (index + 1)
    ...
    const mat2Y = peregorodkaY + (startY + roomHeight - peregorodkaY - mat2H) / 2
  ...
  // Uzunlik
  ctx.fillText(`${joriyXona.value.uzunlik.toFixed(2)} m`, startX + roomWidth / 2 - 35, startY - 15)
  ...
  ctx.setLineDash([])
  ctx.fillStyle = '#000000'
  ctx.fillText(`- Material (-${foizKamayish.value}%)`, legendStartX + 25, legendY2 + 11)
  ...
  ctx.setLineDash([])
  ctx.fillStyle = '#000000'
  ctx.fillText(`- Kemtik material (-${foizKamayish.value}%)`, legendStartX + 185, legendY2 + 11)
...
const xonadonSaqlash = () => {
  ...
  const tanlganXonalar = tanlangan.value.map(id => {
  ...
  <div class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
    <div class="max-w-7xl mx-auto">
      ...
      <div class="bg-white rounded-xl shadow-lg p-6 mb-6">
        <h1 class="text-3xl font-bold text-indigo-900 mb-2">🏠 Натяжной Потолок Калькулятори</h1>
        <p class="text-gray-600">Xonalar, kemtiklar va peregorodkalar bilan professional hisoblash</p>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1 space-y-6">
          ...
          <div class="bg-white rounded-xl shadow-lg p-6">
            <h2 class="text-xl font-bold text-gray-800 mb-4">➕ Yangi Xona Qo'shish</h2>
            
            <div class="space-y-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Xona nomi</label>
                <input 
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Xonaning eni (m)</label>
                <input 
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Xonaning uzunligi (m)</label>
                <input 
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Mavsum</label>
                <select 
                  v-model="mavsumTuri"
                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                >
          ...
          <div class="bg-white rounded-xl shadow-lg p-6">
            <h2 class="text-xl font-bold text-gray-800 mb-4">📋 Mavjud Xonalar ({{ xonalar.length }})</h2>
            
            <div v-if="xonalar.length === 0" class="text-center text-gray-500 py-8">
            ...
            <div v-else class="space-y-2 max-h-96 overflow-y-auto">
              <div 
                ...
                :class="joriyXona?.id === xona.id ? 'bg-indigo-50 border-indigo-500' : 'bg-gray-50 border-gray-200'"
              >
                ...
                <p class="text-sm text-gray-600">
                  {{ xona.uzunlik }}m × {{ xona.kenglik }}m = {{ (xona.uzunlik * xona.kenglik).toFixed(2) }}m²
                </p>
                <p class="text-xs text-gray-500 mt-1">
                  {{ xona.mavsumTuri === 'issiq' ? '☀️ Issiq' : '❄️ Sovuq' }} | 
                  Kemtiklar: {{ xona.kemtiklar.length }} | 
                  Peregorodka: {{ xona.peregorodkalar.length }}
                </p>
              </div>
            </div>
          </div>
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="bg-white rounded-xl shadow-lg p-6">
            <h2 class="text-xl font-bold text-gray-800 mb-4">
              📐 Xona Chizmasi
              <span v-if="joriyXona" class="text-base font-normal text-indigo-600">
                - {{ joriyXona.nom }}
              </span>
            </h2>
            ...
            <div class="grid md:grid-cols-3 gap-4">
              <!-- Chizma (chap tomon) -->
              <div class="md:col-span-2 border-2 border-gray-200 rounded-lg bg-gray-50 overflow-hidden">
                <canvas 
                  ref="canvas" 
                  width="700" 
                  height="500"
              ...
              <div v-if="joriyXona" class="space-y-3">
                <div class="bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg p-4 border-2 border-green-200">
                  <h3 class="text-sm font-bold text-green-800 mb-2">📏 Material O'lchamlari</h3>
                  <div class="space-y-2 text-sm">
                    <div class="bg-white rounded p-2">
                      <p class="text-xs text-gray-600">Uzunlik</p>
                      <p class="font-bold text-green-700">{{ (joriyXona.uzunlik * (1 - (joriyXona.mavsumTuri === 'issiq' ? 0.10 : 0.07))).toFixed(2) }}m</p>
                    </div>
                    <div class="bg-white rounded p-2">
                      <p class="text-xs text-gray-600">Kenglik</p>
                      <p class="font-bold text-green-700">{{ (joriyXona.kenglik * (1 - (joriyXona.mavsumTuri === 'issiq' ? 0.10 : 0.07))).toFixed(2) }}m</p>
                    </div>
                    <div class="bg-white rounded p-2">
                      <p class="text-xs text-gray-600">Jami maydon</p>
                      <p class="font-bold text-green-700">{{ materialMaydon.toFixed(2) }}m²</p>
                    </div>
                    <div class="bg-green-100 rounded p-2">
                      <p class="text-xs text-green-700">Kamayish</p>
                      <p class="font-bold text-green-800">-{{ foizKamayish }}%</p>
                    </div>
                  </div>
                </div>

                <!-- Kemtik masofalar (agar mavjud bo'lsa) -->
                <div v-if="joriyXona.kemtiklar.length > 0" class="bg-gradient-to-br from-red-50 to-orange-50 rounded-lg p-4 border-2 border-red-200">
                  <h3 class="text-sm font-bold text-red-800 mb-2">🔴 Kemtik Masofalar</h3>
                  <div v-for="masofa in kemtikMasofalar" :key="masofa.kemtikId" class="mb-3 last:mb-0">
                    <p class="text-xs font-semibold text-red-700 mb-1">{{ masofa.kemtikNom }}</p>
                    <div class="space-y-1 text-xs">
                      <div class="flex justify-between bg-white rounded px-2 py-1">
                        <span class="text-gray-600">⬅️ Chap:</span>
                        <span class="font-bold text-red-600">{{ masofa.chap }}m</span>
                      </div>
                      <div class="flex justify-between bg-white rounded px-2 py-1">
                        <span class="text-gray-600">⬆️ Yuqori:</span>
                        <span class="font-bold text-orange-600">{{ masofa.yuqori }}m</span>
                      </div>
                      <div class="flex justify-between bg-white rounded px-2 py-1">
                        <span class="text-gray-600">➡️ O'ng:</span>
                        <span class="font-bold text-yellow-600">{{ masofa.ong }}m</span>
                      </div>
                      <div class="flex justify-between bg-white rounded px-2 py-1">
                        <span class="text-gray-600">⬇️ Past:</span>
                        <span class="font-bold text-green-600">{{ masofa.past }}m</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-4 border-2 border-blue-200">
                  <h3 class="text-sm font-bold text-blue-800 mb-2">🏠 Xona O'lchamlari</h3>
                  <div class="space-y-2 text-sm">
                    <div class="bg-white rounded p-2">
                      <p class="text-xs text-gray-600">Uzunlik</p>
                      <p class="font-bold text-blue-700">{{ joriyXona.uzunlik.toFixed(2) }}m</p>
                    </div>
                    <div class="bg-white rounded p-2">
                      <p class="text-xs text-gray-600">Kenglik</p>
                      <p class="font-bold text-blue-700">{{ joriyXona.kenglik.toFixed(2) }}m</p>
                    </div>
                    <div class="bg-white rounded p-2">
                      <p class="text-xs text-gray-600">Maydon</p>
                      <p class="font-bold text-blue-700">{{ xonaMaydon.toFixed(2) }}m²</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div v-if="joriyXona" class="bg-white rounded-xl shadow-lg p-6">
            <h2 class="text-xl font-bold text-gray-800 mb-4">📊 Umumiy Hisoblashlar</h2>
            
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div class="bg-blue-50 rounded-lg p-4">
                <p class="text-sm text-gray-600 mb-1">Xona Maydoni</p>
                <p class="text-2xl font-bold text-blue-600">{{ xonaMaydon.toFixed(2) }} m²</p>
              </div>

              <div class="bg-red-50 rounded-lg p-4">
                <p class="text-sm text-gray-600 mb-1">Kemtiklar</p>
                <p class="text-2xl font-bold text-red-600">{{ kemtiklarMaydon.toFixed(2) }} m²</p>
              </div>

              <div class="bg-green-50 rounded-lg p-4">
                <p class="text-sm text-gray-600 mb-1">Toza Maydon</p>
                <p class="text-2xl font-bold text-green-600">{{ tozaMaydon.toFixed(2) }} m²</p>
              </div>

              <div class="bg-indigo-50 rounded-lg p-4">
                <p class="text-sm text-gray-600 mb-1">Material (-{{ foizKamayish }}%)</p>
                <p class="text-2xl font-bold text-indigo-600">{{ materialMaydon.toFixed(2) }} m²</p>
              </div>
            </div>

            <div v-if="joriyXona.kemtiklar.length > 0 || peregorodkaMaydon > 0" class="grid grid-cols-2 gap-4 mt-4">
              <div v-if="joriyXona.kemtiklar.length > 0" class="bg-orange-50 rounded-lg p-4">
                <p class="text-sm text-gray-600 mb-1">Kemtiklar Material</p>
                <p class="text-xl font-bold text-orange-600">{{ kemtiklarMaterialMaydon.toFixed(2) }} m²</p>
                <p class="text-xs text-gray-500 mt-1">Kemtiklar ichidagi material</p>
              </div>

              <div v-if="peregorodkaMaydon > 0" class="bg-yellow-50 rounded-lg p-4">
                <p class="text-sm text-gray-600 mb-1">Peregorodka Material</p>
                <p class="text-xl font-bold text-yellow-600">{{ peregorodkaMaydon.toFixed(2) }} m²</p>
              </div>
            </div>
          </div>
          ...
          <div v-if="joriyXona" class="grid md:grid-cols-2 gap-6">
            <!-- Kemtik -->
            <div class="bg-white rounded-xl shadow-lg p-6">
              <h3 class="text-lg font-bold text-gray-800 mb-4">🔴 Kemtik Qo'shish</h3>
              ...
              <div class="space-y-3">
                ...
                <div class="bg-red-50 rounded-lg p-3 space-y-2">
                  <p class="text-xs font-semibold text-gray-700">📍 Pozitsiya (metrda, qaysi qismida turishi) </p>
                  
                  <div class="grid grid-cols-2 gap-2">
                    <div>
                      <label class="text-xs text-gray-600">Xonaning chap qismidan (m) (eni)</label>
                      <input 
                    ...
                    <div>
                      <label class="text-xs text-gray-600">Xonaning yuqori qismidan (m) (uzunligi)</label>
                      <input 
                  ...
                  <div v-if="joriyXona" class="bg-white rounded p-2 mt-2">
                    <p class="text-xs font-semibold text-gray-700 mb-1">💡 Misol:</p>
                    <p class="text-xs text-gray-600">
                      Xona: {{ joriyXona.uzunlik }}×{{ joriyXona.kenglik }}m
                    </p>
                    <ul class="text-xs text-gray-600 mt-1 space-y-0.5">
                      <li>• Chap-yuqori: X=0, Y=0</li>
                      <li>• O'ng-yuqori: X={{ (joriyXona.uzunlik - (yangiKemtikUzunlik || 0.5)).toFixed(2) }}, Y=0</li>
                      <li>• Chap-past: X=0, Y={{ (joriyXona.kenglik - (yangiKemtikKenglik || 0.5)).toFixed(2) }}</li>
                      <li>• O'ng-past: X={{ (joriyXona.uzunlik - (yangiKemtikUzunlik || 0.5)).toFixed(2) }}, Y={{ (joriyXona.kenglik - (yangiKemtikKenglik || 0.5)).toFixed(2) }}</li>
                    </ul>
                  </div>
                </div>
              ...
              <div v-if="joriyXona.kemtiklar.length > 0" class="mt-4 space-y-2">
                <h4 class="font-semibold text-gray-700 text-sm">Qo'shilgan kemtiklar:</h4>
                <div 
            ...
            <!-- Peregorodka -->
            <div class="bg-white rounded-xl shadow-lg p-6">
              <h3 class="text-lg font-bold text-gray-800 mb-4">🟡 Peregorodka Qo'shish</h3>
              ...
              <div v-if="joriyXona.peregorodkalar.length > 0" class="mt-4 space-y-2">
                <h4 class="font-semibold text-gray-700 text-sm">Qo'shilgan peregorodkalar:</h4>
                <div 
                  v-for="per in joriyXona.peregorodkalar" 
                  :key="per.id"
                  class="flex justify-between items-center bg-yellow-50 p-2 rounded text-sm"
                >
                  <span>{{ per.nom }} ({{ per.birinchiTomon }}m | {{ per.ikkinchiTomon }}m)</span>
                  <button 
      ...
      <!-- Umumiy hisoblashlar -->
      <div v-if="xonalar.length > 0" class="bg-white rounded-xl shadow-lg p-6 mt-6">
        <h2 class="text-xl font-bold text-gray-800 mb-4">📈 Umumiy Hisoblashlar</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-6 text-white">
            ...
            <p class="text-4xl font-bold">{{ umumiyXonaMaydon.toFixed(2) }} m²</p>
          </div>
          ...
          <div class="bg-gradient-to-br from-indigo-500 to-indigo-600 rounded-xl p-6 text-white">
            ...
            <p class="text-4xl font-bold">{{ umumiyMaterialMaydon.toFixed(2) }} m²</p>
          </div>
        </div>
      </div>
      ...
      <div v-if="xonalar.length > 0" class="bg-white rounded-xl shadow-lg p-6 mt-6">
        ...
        <div class="grid md:grid-cols-2 gap-6">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Xonadon nomi</label>
            <input 
              v-model="xonadonNomi" 
              type="text" 
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Xonalarni tanlang</label>
      ...
      <div v-if="xonadonlar.length > 0" class="bg-white rounded-xl shadow-lg p-6 mt-6">
        <h2 class="text-xl font-bold text-gray-800 mb-4">📚 Saqlangan Xonadonlar</h2>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          >
            <h3 class="font-bold text-lg text-gray-800 mb-2">{{ xonadon.nom }}</h3>
